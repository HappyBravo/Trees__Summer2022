...
avlNode *avl_insert(avlNode *root, int data){
    if (!(root)){ 
        // IF THERE IS NO TREE, WE CREATE THE FIRST NODE AND MAKE IT ROOT OF THE TREE
        avlNode *new_node;
        ...
        if(avl_height(root->right) - avl_height(root->left) == 2){ // RIGHT (SUB)TREE IS HEAVY
        ...
        if(avl_height(root->left) - avl_height(root->right) == 2){ // LEFT (SUB)TREE IS HEAVY
...
int _avl_print_t(avlNode *tree, int is_left, int offset, int depth, char s[20][512])
{
    ...
    sprintf(b, "(%03d,%02d)", tree->data, tree->height);
